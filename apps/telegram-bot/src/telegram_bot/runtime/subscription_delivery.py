    ...
    def _service_text(self, service: ServiceSummary, ready: bool) -> str:
        ...
        if service.status.casefold() == "active" and not ready:
            lines.extend(
                [
                    "",
                    "تحویل لینک یا کانفیگ در حال حاضر قابل تأیید نیست؛ "
                    "کمی بعد وضعیت را بروزرسانی کنید.",
                ]
            )
        elif ready:
            lines.extend(
                [
                    "",
                    "🔐 اطلاعات اتصال محرمانه است و فقط با انتخاب دکمه لینک اشتراک یا "
                    "کانفیگ مستقیم نمایش داده می‌شود.",
                ]
            )
        return "\n".join(lines)
