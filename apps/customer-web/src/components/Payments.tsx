...
function MethodCards({ purpose, selected, onSelect, onAvailability }: { purpose: PaymentPurpose; selected?: string; onSelect: (method: PaymentMethod) => void; onAvailability?: (available: boolean) => void }): React.ReactElement {
  ...
  if (!visible.length) return <section className="payment-state warn topup-unavailable" data-testid="topup-unavailable"><h2>پرداخت آنلاین هنوز فعال نیست</h2><p>در حال حاضر روش پرداخت فعالی برای شارژ حساب وجود ندارد.</p><a href="/support">ارتباط با پشتیبانی</a></section>;
  return <div className="payment-methods" role="radiogroup" aria-label="روش پرداخت">{visible.map((method) => <button key={method.code} className={`payment-method ${selected === method.code ? "selected" : ""}`} role="radio" aria-checked={selected === method.code} onClick={() => onSelect(method)}><strong>{method.display_name}</strong>{method.description ? <span>{method.description}</span> : null}<small>از {formatTomanFromRial(method.minimum_amount_rial)} تا {formatTomanFromRial(method.maximum_amount_rial)}</small></button>)}</div>;
}
...
export function WalletTopupPage(): React.ReactElement {
  ...
  const validation = rial === null ? (amount ? "مبلغ واردشده معتبر نیست." : null) : rial < policy.minimum_topup_amount_rial ? "مبلغ از حداقل شارژ کمتر است." : rial > policy.maximum_topup_amount_rial ? "مبلغ از حداکثر شارژ بیشتر است." : policy.maximum_wallet_balance_rial && summary.available_balance_rial + rial > policy.maximum_wallet_balance_rial ? "این مبلغ از سقف موجودی بیشتر می‌شود." : null;
  const suggestions = [50000, 100000, 250000, 500000, 1000000].filter((candidate) => { const candidateRial = tomanToExactRial(candidate); return candidateRial >= policy.minimum_topup_amount_rial && candidateRial <= policy.maximum_topup_amount_rial && (!policy.maximum_wallet_balance_rial || summary.available_balance_rial + candidateRial <= policy.maximum_wallet_balance_rial); });
  ...
  return <main className="wallet-page topup-page"><WalletNavigation current="top-up" /><header><p className="eyebrow">کیف پول</p><h1>افزایش موجودی</h1></header><section className="current-balance"><span>موجودی فعلی</span><TomanAmount value={summary.available_balance_rial} /></section><section className="topup-amount"><label htmlFor="topup-amount">مبلغ شارژ</label><div className="amount-input"><input id="topup-amount" inputMode="numeric" autoComplete="off" value={amount} onChange={(event) => changeAmount(event.currentTarget.value)} aria-describedby="amount-currency amount-limits" /><span id="amount-currency">تومان</span><button type="button" onClick={() => changeAmount("")}>پاک کردن</button></div><div className="quick-amounts" aria-label="مبلغ‌های پیشنهادی">{suggestions.map((candidate) => <button key={candidate} type="button" aria-pressed={toman === candidate} onClick={() => changeAmount(String(candidate))}>{formatToman(candidate)}</button>)}</div><p id="amount-limits" className="topup-limits">حداقل {formatTomanFromRial(policy.minimum_topup_amount_rial)} · حداکثر {formatTomanFromRial(policy.maximum_topup_amount_rial)}</p>{validation ? <p className="danger-text">{validation}</p> : null}</section><section><h2>روش پرداخت</h2><MethodCards purpose="WALLET_TOPUP" selected={method?.code} onSelect={(selected) => { setMethod(selected); setReview(false); }} onAvailability={setHasMethods} /></section>{hasMethods && method && rial !== null && !validation ? review ? <section className="payment-review"><h2>بازبینی شارژ</h2><p>مبلغ: <TomanAmount value={rial} /></p><p>روش: {method.display_name}</p><p>موجودی پس از تأیید: <TomanAmount value={summary.available_balance_rial + rial} /></p><small>موجودی پس از تأیید نهایی پرداخت تغییر می‌کند.</small><button disabled={busy} onClick={() => void submit()}>{busy ? "در حال ثبت…" : "تأیید و ادامه"}</button></section> : <button className="button topup-review" onClick={() => setReview(true)}>ادامه و بازبینی</button> : null}</main>;
}
