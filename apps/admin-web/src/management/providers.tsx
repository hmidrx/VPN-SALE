...
export function ProviderDashboard(): React.ReactElement {
  return <section className="panel stack" dir="rtl"><h2>هسته ارائه‌دهندگان VPN</h2><p>این کنسول فقط برای اتصال امن، تشخیص نسخه، قابلیت‌ها و همگام‌سازی read-only است.</p><div className="cards"><article className="metric"><span>نسخه‌های certified</span><strong>3</strong><small className="tech">3x-ui v3.5.0 / x-ui v1.11.3 / PasarGuard v5.1.0</small></article><article className="metric"><span>وضعیت live</span><strong>LIVE_UNVERIFIED</strong><small>تا اجرای certification روی staging واقعی.</small></article><article className="metric"><span>عملیات نوشتن</span><strong>غیرفعال</strong><small className="tech">PROVIDER_OPERATION_NOT_ENABLED</small></article></div></section>;
}
...
export function ProviderCapabilityMatrix(): React.ReactElement {
  const rows = ["PANEL_VERSION_READ", "NODE_LIST", "INBOUND_LIST", "CLIENT_LIST", "HOST_LIST", "CLIENT_CREATE"];
