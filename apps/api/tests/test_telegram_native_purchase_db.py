...
async def test_fulfillment_is_exactly_once_and_visible_to_bot_and_service_projection(
    ...
    assert status_response.json()["expires_at"] is None
    assert status_response.json()["service_lifecycle"] == "PENDING_ACTIVATION"
    assert status_response.json()["delivery_ready"] is False
...
@pytest.mark.asyncio
async def test_duplicate_outbox_delivery_converges_to_one_attempt_and_service(
    purchase_app: PurchaseApp,
) -> None:
    app, factory, _ = purchase_app
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://private") as client:
        ...
        await client.post(
            "/api/v1/internal/telegram/purchase/confirm",
            headers=headers("duplicate-event-key"),
            json=purchase_body(item),
        )
    with factory.begin() as db:
        original = db.scalar(select(TransactionalOutboxModel))
        assert original is not None
        db.add(
            TransactionalOutboxModel(
                event_key=f"duplicate:{original.id}",
    ...
    provider = SuccessfulProvisioner()
    worker = OrderFulfillmentWorker(factory, provider, "duplicate-worker")
    assert worker.run_once() == 1
    assert provider.calls == 1
    with factory() as db:
        assert db.scalar(select(func.count()).select_from(ServiceModel)) == 1
...
@pytest.mark.asyncio
async def test_writes_disabled_releases_claim_without_service(purchase_app: PurchaseApp) -> None:
    app, factory, _ = purchase_app
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://private") as client:
        item = await plan(client)
        await client.post(
            "/api/v1/internal/telegram/purchase/confirm",
            headers=headers("disabled-write-key"),
            json=purchase_body(item),
        )
    worker = OrderFulfillmentWorker(factory, DisabledProvisioner(), "disabled-worker")
    assert worker.run_once() == 1
    with factory() as db:
        event = db.scalar(select(TransactionalOutboxModel))
        assert event is not None and event.status == "PENDING" and event.claimed_at is None
