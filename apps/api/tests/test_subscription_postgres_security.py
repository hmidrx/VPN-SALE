...
def _seed(factory: sessionmaker[Session]) -> None:
    ...
    fingerprint = __import__("hashlib").sha256(REMOTE_IDENTITY.encode()).hexdigest()
        ...
        db.execute(
            text(
                ...
                INSERT INTO services (
                    ...
                    jsonb_build_object('product_version_id', :product_version), '{}'::jsonb,
                    :now, :expires, :now, :now, 1
                )
                """
            ),
            {
        ...
        db.execute(
            text(
                ...
                INSERT INTO delivery_revisions (
                    ...
                    jsonb_build_object('URI', :renderer),
                    jsonb_build_object(:attachment_id, :fingerprint),
...
def test_postgres_subscription_issue_render_rotate_and_revoke_are_hash_only() -> None:
    ...
    try:
        ...
        with factory() as db:
            ...
            token_row = db.scalar(select(DeliverySubscriptionTokenModel))
            assert token_row is not None
            assert token_row.token_hash.startswith("sha256:")
            assert first_token not in token_row.token_hash
            assert subscription.active_token_hash == token_row.token_hash
            ...
            decoded = base64.b64decode(base64_links).decode()
            assert decoded == plain
            assert "edge.example" in mihomo
            assert "panel" not in mihomo.lower()
            parsed_sing_box = json.loads(sing_box)
            assert parsed_sing_box["outbounds"]
            ...
            assert second_token not in serialized_events
