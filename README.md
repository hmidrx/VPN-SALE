...
The repository currently supports a temporary no-lockfile npm path. The first successful Codespaces bootstrap or frontend CI run may generate `package-lock.json`. If CI creates it, download the `generated-package-lock` artifact from the workflow run and upload it to a new GitHub branch using **Add file > Upload files** in the browser. If Codespaces creates it, commit it with:
