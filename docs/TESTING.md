...
The July 2026 Milestone 0 dependency remediation upgraded the web apps from `next@15.1.3`, `react@19.0.0`, and `react-dom@19.0.0` to patched React 19.2 runtime packages and then to `next@16.3.0-canary.93` when the CI high-severity audit gate began flagging Next's transitive `postcss<8.5.10` and `sharp<0.35.0` dependency graph. React and React DOM remain on `19.2.7`, Node type packages stay on the Node 22 line, and React type packages remain on the compatible 19.0.x line required by generated validator types. Validation commands for dependency remediation include `npm ci`, `npm audit --audit-level=high`, `npm ls next sharp postcss`, workspace lint/typecheck/test, the three real Next.js production builds, Docker frontend image builds where Docker is available, backend Ruff/Pyright/pytest checks, the repository security scan, the security scanner regression test, and `git diff --check`. Dependency-selection details are documented in `docs/SECURITY.md`.
...
The customer wallet presents money only in toman and uses a compact RTL layout across Telegram safe areas. Wallet top-up requires at least 100,000 toman (1,000,000 canonical rial); internal upper bounds remain server-enforced but are intentionally absent from customer copy. When no payment method is active, the UI remains fail-closed, offers support, and cannot submit a top-up intent. Migration downgrade restores the legacy 100,000-rial value (10,000 toman), while reverting the customer UI assets restores the earlier presentation; no ledger data is rewritten.
