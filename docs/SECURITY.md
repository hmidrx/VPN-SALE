...
The first remediated Milestone 0 baseline pinned each web app to `next@15.5.20`, `react@19.2.7`, `react-dom@19.2.7`, `@types/react@19.0.2`, `@types/react-dom@19.0.2`, and `@types/node@22.20.1`. That kept the project on the patched Next.js 15 line while avoiding an unnecessary major-version migration at the time. The React runtime packages were upgraded to patched 19.2.x releases; React type packages remained on the compatible 19.0.x line because Next.js generated validation types reference the global `React.ComponentType` namespace shape. Each web app includes a small `react-global.d.ts` compatibility shim for that generated validator while retaining strict TypeScript checks.

A later July 2026 frontend audit gate started failing because the latest stable and backport Next.js releases still pinned vulnerable transitive versions: `next@15.5.21` and `next@16.2.11` both declared `postcss@8.4.31`, and the stable 16 line still allowed `sharp@0.34.x`. The automated npm fix proposed a breaking downgrade to `next@9.3.3`, which is explicitly rejected. The nearest published Next.js dependency graph that resolves both audit findings is `next@16.3.0-canary.93`, which declares `postcss@8.5.10` and `sharp@^0.35.3` while retaining the existing React peer range that includes `react@19.2.7` and `react-dom@19.2.7`. The upgrade is pinned exactly in all three web apps, keeps React/React DOM unchanged, avoids npm overrides, and is verified with `npm ci`, `npm audit --audit-level=high`, workspace lint/typecheck/tests, all three production web builds, and repository security scans.

Final `npm audit --audit-level=high` status after the upgrade: zero vulnerabilities.
