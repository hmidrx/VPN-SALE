...
Provider creation alone does not start the customer entitlement. While delivery remains
pending, the service's customer-visible `starts_at`, `activated_at`, and `expires_at` are
null. The fulfillment entitlement clock is provider-operation state only; an activation
path must establish the customer-visible timestamps atomically with usable delivery.
...
Downgrading revision 0037 backfills any still-null legacy `services.starts_at` from
`created_at` solely to restore the older non-null schema, so writes must remain disabled
during that rollback and operators must review pending services before re-enabling them.
