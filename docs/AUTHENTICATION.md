...
The Telegram bot customer menu is permanently Persian-only. Current working destinations are buy service, services, profile, wallet, sessions/security, support, education, system status, announcements, privacy and help; language switching is removed and stale language callbacks are handled safely in Persian. Future commerce modules must register commands and menu items through feature modules and must not place product, pricing, payment or provisioning rules inside bot handlers.
