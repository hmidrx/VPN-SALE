...
The first successful Codespaces bootstrap or frontend CI run may generate `package-lock.json`. If CI generated it, open the workflow run, download the `generated-package-lock` artifact, create a new branch in the GitHub web UI, choose **Add file > Upload files**, upload `package-lock.json`, and commit it to that branch. If Codespaces generated it, review and commit it exactly with:
