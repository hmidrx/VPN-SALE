"""Read-only certified adapter for pasarguard."""
...
class PasarGuardAdapter:
    definition = ProviderAdapterVersion("pasarguard", "0.6a1", ProviderKind.PASARGUARD)
    contract = CERTIFIED_CONTRACTS[ProviderKind.PASARGUARD]
    capabilities = capability_matrix(
        set(READ_COMMON)
        | {
            ProviderCapability.NODE_LIST,
            ProviderCapability.HOST_LIST,
            ProviderCapability.TEMPLATE_LIST,
            ProviderCapability.HWID_METADATA_READ,
            ProviderCapability.PERIODIC_TRAFFIC_DISCOVERY,
    ...
    async def detect_version(self, transport: SecureHttpTransport) -> PanelVersionDetection:
        response = await transport.get("/api/system/status")
        ...
        return PanelVersionDetection(
            ProviderKind.PASARGUARD, version, status, self.contract.contract_digest, self.definition
        )
    ...
    async def fetch_inventory(
        ...
        nodes_response = await transport.get("/api/nodes")
        inbounds_response = await transport.get("/api/users")
        nodes = tuple(
