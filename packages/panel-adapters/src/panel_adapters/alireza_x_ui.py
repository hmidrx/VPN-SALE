"""Read-only certified adapter for alireza_x_ui."""
...
class AlirezaXuiAdapter:
    definition = ProviderAdapterVersion("alireza_x_ui", "0.6a1", ProviderKind.ALIREZA_X_UI)
    contract = CERTIFIED_CONTRACTS[ProviderKind.ALIREZA_X_UI]
    capabilities = capability_matrix(
        set(READ_COMMON)
        | {ProviderCapability.CLIENT_ONLINE_READ, ProviderCapability.FIRST_USE_EXPIRY_DISCOVERY},
        "0.6a1",
        "docs/provider-contracts/alireza-x-ui/v1.11.3/contract.md",
    )

    async def detect_version(self, transport: SecureHttpTransport) -> PanelVersionDetection:
        response = await transport.get("/xui/API/server/status")
