...
grep -Fq 'docker compose run --rm --no-deps --entrypoint sh api' "$verify"
grep -Fq 'docker compose --profile telegram run --rm --no-deps --entrypoint sh telegram-bot' "$verify"
if grep -Eq 'docker compose( --profile telegram)? images -q (api|telegram-bot)' "$verify"; then exit 1; fi
