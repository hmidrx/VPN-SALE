...
# These two files intentionally contain the scanner/checker regex definitions. Keep the
# exclusion narrow so application, workflow, Docker, and configuration files stay scanned.
...
report_match "Subscription URL" '(vless://|vmess://|trojan://|subscription://)' . "${intentional_pattern_files[@]}"
report_match "Obvious assigned secret" '(api[_-]?key|secret|token|cookie|password)[[:space:]]*[:=][[:space:]]*[^[:space:]]{8,}' . "${intentional_pattern_files[@]}"
report_match "Panel credential marker" '(xui|pasarguard).*(password|secret|token|api[_-]?key)' . "${intentional_pattern_files[@]}"
